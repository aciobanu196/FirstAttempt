...
class CartService extends QueryVariable with CartDao[Cart, Future] {
  ...
  override def insertCart(row: Cart)(implicit db: backend.Database): Future[Cart] =
    db.run(
     carts returning carts
       .map(_.id) into ((row, id) => row.copy(id = id)) += row)

  override def updateProduct(id: Int, row: Cart)(implicit db: backend.Database): Future[Int] = ???
